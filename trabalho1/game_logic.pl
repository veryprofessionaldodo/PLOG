...
check_piece_warfare(Move) :- 
		...
		get_piece(Column, Row, Piece), 
		player_letter(Player, Piece),  
        ((equal(Player, 1), Enemy = 2) ; 
         	(equal(Player, 2), Enemy = 1)),     % Sees which piece is it's enemy
...
check_surroundings(Player, Enemy, Column, Row) :-  
		(((NewColumn1 is Column + 1,
			get_piece(NewColumn1, Row, Piece),							  % Analyze all 4 sides of piece
		  	...
		  	equal(N, Enemy), !, 
		  	is_in_previous_warfare(NewColumn1, Row, Player, Column, Row)) ;  % If it finds an enemy, verifies if 
		(NewColumn2 is Column - 1 , 
		  	get_piece(NewColumn2, Row, Piece),                            % it's already in a warfare with an ally.
		  	player_letter(N, Piece), 
		  	equal(N, Enemy), !, 
		  	is_in_previous_warfare(NewColumn2, Row, Player, Column, Row))) ; 
		((NewRow1 is Row + 1, 
		  	get_piece(Column, NewRow1, Piece),
		  	player_letter(N, Piece), 
		  	equal(N, Enemy), !, 
		  	is_in_previous_warfare(Column, NewRow1, Player, Column, Row)) ;  
		 (NewRow2 is Row - 1 , 
	 	  	get_piece(Column, NewRow2, Piece),
		   	player_letter(N, Piece), 
		   	equal(N, Enemy), !, 
		   	is_in_previous_warfare(Column, NewRow2, Player, Column, Row)))). % If not, then the piece can move.
